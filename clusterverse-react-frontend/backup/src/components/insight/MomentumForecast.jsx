...
export default function MomentumForecast() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        console.warn("⚠️ Forecast load failed:", err);
      }
    }
    load();
  }, []);
  ...
  const message =
    forecast.avg_cohesion > 80
      ? "Strong alignment — teams emotionally synchronized."
      : forecast.avg_cohesion > 60
      ? "Momentum steady — collaboration balanced."
      : "Low emotional drive — potential fatigue phase.";
