// ============================================================
// ðŸ’Ž Core4.AI â€“ TribeEconomyForecast.jsx (v163.2 â€œSmoothed Forecast + Confidence Scoreâ€)
// ------------------------------------------------------------
// âœ… Predicts short-term direction of tribe tokenValue
// âœ… Uses smoothed momentum vs volatility ratio
// âœ… Adds confidence score & color-coded trend strength bar
// ============================================================
...
function computeForecast(history, tribe) {
  ...
  if (recent.length < 3) return { dir: "â†’", delta: 0, confidence: 0 };
  ...
  if (normalized > 0.2) return { dir: "â†‘", delta: normalized, confidence };
  if (normalized < -0.2) return { dir: "â†“", delta: normalized, confidence };
  return { dir: "â†’", delta: 0, confidence };
}
...
export default function TribeEconomyForecast() {
  ...
  // ðŸ§  capture tokenValue history snapshot
  useEffect(() => {
  ...
  // ðŸ”® recompute forecast when history updates
  useEffect(() => {
    const next = {};
    tribes.forEach((t) => {
  ...
  return (
    <div className="bg-[#111827] border border-gray-700 rounded-2xl p-6 shadow-lg">
      <h3 className="text-pink-400 font-semibold mb-3">
        ðŸ¤– Tribe Token Forecast (Next Cycle)
      </h3>
