...
import { useTribeChallenges } from "@/context/TribeChallengeContext";

import TribeFunnelHeatmap from "./TribeFunnelHeatmap";
...
export default function TribeDashboard() {
  ...
  const { challenges = [] } = useTribeChallenges();
  ...
  const aiSuggestion =
    product && clusters.length > 0
      ? autoOptimizeCampaign({
          product,
          clusters,
          ambassadors: getTopAmbassadors(product, 3),
          previousRevenue: logs.reduce((sum, l) => sum + (l.amount || 0), 0),
          conversionEvents: logs.filter(
            (l) => l.funnelStage === "conversion"
          ).length,
        })
      : null;
  ...
  return (
    ...
    >
      ...
      <div className="bg-white/10 border border-white/20 rounded-2xl p-6">
        ...
        <p className="mt-1 text-gray-300">نسبة التقدم: {lifecycleProgress}%</p>
      ...
      <div className="bg-white/10 border border-white/20 rounded-2xl p-6">
        ...
        {aiSuggestion ? (
          <div className="space-y-2 text-gray-200 text-sm">
            <p>
              👤 المؤثر التالي:{" "}
              <b>{aiSuggestion.nextAmbassador}</b>
            </p>
            <p>
              🎯 الشريحة: <b>{aiSuggestion.targetCluster}</b>
            </p>
            <p>
              💰 تعديل السعر: <b>{aiSuggestion.priceAdjustment}</b>
            </p>
            <p>
              🎬 المحتوى القادم: <b>{aiSuggestion.nextContent}</b>
            </p>
